import { findReturnStrReg, findSvgTitleReg, findHeightWidthReg, findViewBoxReg } from '../regex'

const extractSvgHeightWidth = (svgTagStr: string) => {
  let width = '0'
  let height = '0'
  const res = svgTagStr.replace(findHeightWidthReg, (type: string, value: string) => {
    if (type === 'width') {
      width = value
    } else if (type === 'height') {
      height = value
...
export const formatSvgTagAsSymbolTag = (svgStr: string, filename: string, idPrefix = '') => {
  const id = idPrefix ? `${idPrefix}-${getSvgName(filename)}` : getSvgName(filename)
  return svgStr
    .replace(findSvgTitleReg, (svgTagStr: string) => {
